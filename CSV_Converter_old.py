...
class DDtoTM():
    ...
    def Geo2Tm(self, plon, plat):
        ...
        m_dDstMl0 = m_arMajor * (
                m_dDstE0 * m_arLatCenter - m_dDstE1 * math.sin(2.0 * m_arLatCenter) + m_dDstE2 * math.sin(
            4.0 * m_arLatCenter) - m_dDstE3 * math.sin(6.0 * m_arLatCenter))
        ...
        ml = m_arMajor * (m_dDstE0 * lat - m_dDstE1 * math.sin(2.0 * lat) + m_dDstE2 * math.sin(
            4.0 * lat) - m_dDstE3 * math.sin(6.0 * lat))

        x = m_arScaleFactor * n * al * (1.0 + als / 6.0 * (1.0 - t + c + als / 20.0 * (
                5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * m_dDstEsp))) + m_arFalseEasting
        y = m_arScaleFactor * (ml - m_dDstMl0 + n * tq * (als * (0.5 + als / 24.0 * (
